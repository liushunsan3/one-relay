...
### 分钟级限速节流（rpm）+ 两个真 bug 修复
- **实测确认**：商汤 `deepseek-v4-flash` 配额是 **1 RPM**（每分钟 1 次）——第 1 个请求 200、同一分钟第 2 个 429、跨分钟后恢复 200。
- **新增 `rpm` 字段**：`providers.json` 每站可选配 `"rpm": N`（每分钟请求上限），与已有的 `rps`（每秒）组成**双窗口节流**，两个限制取最严。商汤/商汤1 配 `rpm: 1`。
- **修复节流漏洞（关键）**：原实现只在排序时把满站"垫底"但不排除，请求循环仍会依次尝试列表里所有站 → 满站照样被打、互相 429。现改为：**循环前过滤掉已达限速上限的站**；若所有候选都满，**快速返回 429 明确提示**（`所有站均已达限速上限 [商汤(1/分)…]`）而不是硬打制造一串 429。
- **修复 `findProviders` 大小写 bug（重要）**：匹配忽略大小写，但转发用的是**请求的原始（小写）名**而非**站配置里的真实名字**。amd 的 `DeepSeek-V4-Flash` 严格区分大小写 → 小写转发被判 404 `Model not available`（此前 amd 一直失败的真因）。现改为转发**配置里的原始模型名**。
- **前端**：Provider 页「限速」列显示 `当前/上限`（每秒、每分钟各一段）；编辑弹窗支持同时设置 rps/rpm；status 接口新增 `rpm`/`rpmNow`。
- **端到端验证**：并发 6 个 `deepseek-v4-flash` → **6/6 全成功、零 429**，日志显示精确分流（商汤 1 + 商汤1 1 + amd 4，各自遵守限速）。
- 测试增至 **86 项**（新增 findProviders 大小写/别名转发断言防回归）。
